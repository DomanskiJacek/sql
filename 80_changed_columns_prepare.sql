...
BEGIN
EXECUTE IMMEDIATE 'DROP TABLE t4t_change_cols';
    EXCEPTION
        WHEN OTHERS THEN
            NULL;
END;
...
vsql := q'[
CREATE TABLE t4t_change_cols AS
WITH tta AS (
...
vsql := q'[
UPDATE t4t_change_cols SET value_b=100, value_c=null WHERE prod_id='PRODUCT1' AND cdate=date'2024-11-03'
]';
-- UPDATE t4t_change_cols SET value_a=null WHERE prod_id='PRODUCT1' AND cdate<>date'2024-09-03'
